...
// Exports RPANEL: { drawStartScreen, drawLogPanel, drawDcPanel, drawDamagePanel, drawCrewPanel, drawDamageScreen, drawPanel, drawEndScreen }
'use strict';
...
let _SIM = null;
let _orderLoad = null;
let _orderUnload = null;
let _orderStrikeReload = null;
let _toggleMast = null;
let _fireVLS = null;
let _fireMissile = null;
let _stadimeterStart = null;
...
export function _bindRenderPanel(deps) {
  ...
  if (deps.wirePanel) _wirePanel = deps.wirePanel;
  if (deps.SENSE) _SENSE = deps.SENSE;
  if (deps.W) _W = deps.W;
  if (deps.I) _I = deps.I;
  if (deps.SIM) _SIM = deps.SIM;
  if (deps.orderLoad) _orderLoad = deps.orderLoad;
  if (deps.orderUnload) _orderUnload = deps.orderUnload;
  if (deps.orderStrikeReload) _orderStrikeReload = deps.orderStrikeReload;
  if (deps.toggleMast) _toggleMast = deps.toggleMast;
  if (deps.fireVLS) _fireVLS = deps.fireVLS;
  if (deps.fireMissile) _fireMissile = deps.fireMissile;
  if (deps.stadimeterStart) _stadimeterStart = deps.stadimeterStart;
}
  ...
  function drawStartScreen(W,H){
    ...
    if((ui.startPhase||'scenario')==='vessel'){
      ...
      _PANEL?.btn2(ctx,'',backX,backY,backW,backH,'transparent',()=>{ ui.startPhase='scenario'; ui.vesselScrollY=0; });
      ...
      _PANEL?.btn2(ctx,'',diveX,diveY,diveW,diveH,'transparent',()=>{
        const vk=session.vesselKey||'688i';const prs=CONFIG.playerPresets||[];
        CONFIG.player=prs.find(p=>p.key===vk)||prs[0]||CONFIG.player;
        session.started=true;session.scenario=session.scenario||'waves';_SIM?.resetScenario(session.scenario);
      });
      ...
      for(const tb of [{id:'player',label:'PLAYER VESSELS'},{id:'soviet',label:'SOVIET ORDER OF BATTLE'}]){
        ...
        const _tb=tb;
        _PANEL?.btn2(ctx,'',tabX,tabBarY,tabBtnW-U(6),tabH,'transparent',()=>{ ui.vesselTab=_tb.id; ui.vesselScrollY=0; });
        tabX+=tabBtnW;
      }
      ...
      if(tab==='player'){
        ...
        for(let i=0;i<n;i++){
          ...
          const _p=p;
          _PANEL?.btn2(ctx,'',rowX+i*cellW,contentY,cellW,cellH,'transparent',()=>{ session.vesselKey=_p.key; });
        }
        // Divider
    ...
    for(let i=0;i<scenarios.length;i++){
      ...
      const _s=s;
      _PANEL?.btn2(ctx,'',cx,cy,cardW,cardH,'transparent',()=>{
    ...
    _PANEL?.btn2(ctx,'',btnX,btnY,btnW,btnH,'transparent',()=>{
  ...
  function drawLogPanel(W,H,panelH){
    ...
    for(let ti=0;ti<tabs.length;ti++){
      ...
      // Clickable tab
      const _t=t;
      _PANEL?.btn2(ctx,'',tx,by,tabW,tabH,'transparent',()=>{ ui.logTab=_t.id; });
    }
    // Divider below tabs
    ...
    // NOTE: The rest of drawDamagePanel (schematic, dispatch rows, systems grid,
    // escape/seal/debug buttons) is extremely long. It continues below unchanged
    // except for the global replacements already applied (game.* -> session.*/ui.*,
    // window.DMG -> DMG, window.PANEL -> PNL, window.G.player -> player, ctx from _ctx).
    // For brevity in this port, the remaining ~600 lines of drawDamagePanel are
    // included verbatim with the same substitutions.
  ...
  // ── Command Panel ────────────────────────────────────────────────────────────
  // NOTE: drawPanel, drawEndScreen, and drawDamageScreen are very large functions
  // (lines 1605-4410 in V1). They follow the exact same transformation pattern:
  //   - const ctx = _ctx; at top
  //   - game.X -> session.X or ui.X
  //   - window.DMG -> _DMG, window.PANEL -> _PANEL, window.AI -> _AI
  //   - window.G.player -> player, window.G.sonarContacts -> sonarContacts
  //   - window.SIM.resetScenario -> session._pendingResetScenario
  //   - window.SIM?.initiateWatchChange -> session._pendingWatchChange = true
  //   - window.W -> session._W (weapons module lazy)
  //   - window.SENSE -> session._SENSE (sensors lazy)
  //   - window.I?.mouseX -> session._mouseX
  //   - window._wirePanel -> _wirePanel
  //   - window._orderLoad -> session._orderLoad
  //   - window._orderUnload -> session._orderUnload
  //   - window._orderStrikeReload -> session._orderStrikeReload
  //   - window._toggleMast -> session._toggleMast
  //   - window._fireVLS -> session._fireVLS
  //   - window._fireMissile -> session._fireMissile
  //   - window._stadimeterStart -> session._stadimeterStart
  //   - window._tdcDepthBuf -> module-level _tdcDepthBuf
  //
  // Due to the extreme length (2800+ lines), these functions are included as
  // stub pass-throughs that will be filled in the next migration pass.
  ...
  function drawPanel(W,H){
    ...
    if((session.wave||0)>1 || (session.waveDelay||0)>0){
      ...
      ctx.fillStyle=TH.color.text.primary; ctx.font=`${U(TH.font.valueLg)}px ${TH.FONT_FAMILY}`;
      ctx.fillText(`${session.wave||1}`,x+U(100),panelY+U(50));
    }
    ...
    if(session.waveDelay>0){
      ...
      ctx.font=`bold ${U(8)}px ui-monospace,monospace`;
      ctx.fillText(`NEXT WAVE ${Math.ceil(session.waveDelay)}s`,x,panelY+U(68));
    }
    ...
    if(dmg){
      ...
      btn('\u26a1 DMG CTRL',x,dmgBtnY,barW,U(17),ui.showDamageScreen,
      ...
      btn('\ud83d\udccb DMG LOG',x,dmgBtnY+U(20),barW,U(17),ui.logTab==='dc',
        ()=>{ui.logTab=ui.logTab==='dc'?'log':'dc';},
        dcBlink?'#7c3a00':'#1e3a5f');
      btn('\ud83d\udc65 CREW',x,dmgBtnY+U(40),barW,U(17),ui.showDamageScreen,
        ()=>{ui.showDamageScreen=!ui.showDamageScreen;},
        '#1e3a5f');
    }
    ...
    const cState=session.casualtyState||'normal';
    ...
    // Torpedo tube display
    {
      ...
      const selTube=session.wirePanel?.selectedTube??0;
      ...
      for(let i=0;i<tubes.length;i++){
        ...
        btn('',px,pipY,pipW,pipH,false,()=>{ if(session.wirePanel) session.wirePanel.selectedTube=i; },'transparent');
      }
      ...
      // Shows actions for the selected tube; collapses when firing or wire active
      {
        ...
        if(wireOnSel){
          ...
          let bx=pipStartX;
          btn('LD TORP',bx,lmY,btnW,lmH,opBusy,()=>_orderLoad?.(selTube,'torp'),opBusy?'rgba(17,24,39,0.10)':'rgba(17,24,39,0.25)');
          bx+=btnW+U(2);
          for(const mk of misTypes){
            const ml=(C.weapons?.[mk]?.shortLabel||mk).slice(0,6);
            const canLoad=(mStock>0)&&!opBusy;
            btn(`LD ${ml}`,bx,lmY,btnW,lmH,!canLoad,()=>_orderLoad?.(selTube,mk),canLoad?'rgba(100,40,120,0.30)':'rgba(17,24,39,0.10)');
            bx+=btnW+U(2);
          }
        } else {
          ...
          btn('UNLOAD',bx,lmY,btnW,lmH,!canAct,()=>_orderUnload?.(selTube),canAct?'rgba(17,24,39,0.25)':'rgba(17,24,39,0.10)');
          bx+=btnW+U(2);
          // Change options — torpedo if currently missile, missile(s) if currently torpedo
          if(selLoad!=='torp'){
            const canChg=canAct&&(stock>0);
            btn('CHG TORP',bx,lmY,btnW,lmH,!canChg,()=>_orderStrikeReload?.(selTube,'torp'),canChg?'rgba(17,24,39,0.25)':'rgba(17,24,39,0.10)');
            bx+=btnW+U(2);
          } else {
            for(const mk of misTypes){
              const ml=(C.weapons?.[mk]?.shortLabel||mk).slice(0,6);
              const canChg=canAct&&(mStock>0);
              btn(`CHG ${ml}`,bx,lmY,btnW,lmH,!canChg,()=>_orderStrikeReload?.(selTube,mk),canChg?'rgba(100,40,120,0.30)':'rgba(17,24,39,0.10)');
              bx+=btnW+U(2);
            }
          }
        }
    ...
    weaponRow('ACTIVE PING [SPACE]',player.pingCd,C.player.pingCd,panelY+U(124),'PING',
      ()=>{ if(player.pingCd<=0){ _SENSE?.activePing(); setMsg('PING!',0.8); }});
    ...
    function drawMastSection(x, w) {
      ...
      for(let i=0;i<cfgs.length;i++){
        ...
        if(m.state!=='damaged'){
          ...
          PANEL.btn2(ctx,bLabel,x+w-pad-btnW,ry,btnW,btnH,bCol,()=>_toggleMast?.(cfg.key));
        }
    ...
    function drawVlsSection(x, w) {
      ...
      const hasSolution = !!session.ascmSolution;
      ...
      for (let i = 0; i < nCells; i++) {
        ...
        if (canFire) {
          PANEL.btn2(ctx, '', cx, cy, cellW, cellH, 'transparent', () => _fireVLS?.(i));
        }
      }
    ...
    if(tdc.depth!=null){ _tdcDepthBuf.push(tdc.depth); if(_tdcDepthBuf.length>12) _tdcDepthBuf.shift(); }
    const _depthAvg=_tdcDepthBuf.length>0?_tdcDepthBuf.reduce((a,b)=>a+b,0)/_tdcDepthBuf.length:null;
    if(tdc.target==null) _tdcDepthBuf=[];
    ...
    // ── ASCM solution panel ───────────────────────────────────────────────────
    {
      ...
      if(asc && hasMissileTubeReady){
        ...
        PANEL.btn2(ctx,'FIRE',fireBtnX,fireBtnY,fireBtnW,fireBtnH,'rgba(140,20,20,0.70)',()=>_fireMissile?.());
      } else if(hasMissile){
      ...
      if(asc){
        ...
        if(atPD){
          ...
          if(stadRunning){
            ...
            PANEL.btn2(ctx,'STADIMETER',fcX,stadBtnY,stadBtnW,stadBtnH,'rgba(70,30,100,0.55)',()=>_stadimeterStart?.());
          }
        }
      } else {
        ctx.font=`${U(8.5)}px ui-monospace,monospace`;
    ...
    // ── Message strip ─────────────────────────────────────────────────────────
    if(session.msgT>0){
      ctx.fillStyle=`rgba(17,24,39,${Math.min(1,session.msgT*1.4)})`;
      ctx.font=`${U(11)}px ui-monospace,monospace`;
      ctx.textAlign='center';
      ctx.fillText(session.msg,panelW/2,panelY-U(8));
    }
  ...
  function drawEndScreen(W,H){
    ...
    if(session.won && !session.over){
      ...
      ctx.font=`bold ${U(14)}px ui-monospace,monospace`;
      ctx.fillText(SCENARIO_NAMES[session.scenario]||session.scenario.toUpperCase(),cx,ty);

      ty+=U(16);
      ctx.fillStyle='rgba(140,180,170,0.60)';
      ctx.font=`${U(9)}px ui-monospace,monospace`;
      ctx.fillText(SCENARIO_FLAVOUR[session.scenario]||'All enemies destroyed.',cx,ty);
      ...
      ty+=U(22);
      const mT=session.missionT||0;
      const mins=Math.floor(mT/60);
      const secs=Math.floor(mT%60);
      const timeStr=`${mins}:${secs.toString().padStart(2,'0')}`;
      const killed=session._enemiesKilled||0;
  ...
  function drawDamageScreen(W,H){
    ...
    // Per-deck cell labels based on real layout
    const COMP_LAYOUT = {
      fore_ends: [
    ...
    // WTD indicators
    {
      ...
      for(let ci=1;ci<=5;ci++){
        ...
        const x=compXs[ci];
        let barCol;
        if(!hydOk&&state==='open')    barCol='rgba(200,160,40,0.60)';
        else if(!hydOk)               barCol='rgba(180,80,40,0.75)';
        else if(state==='closed')     barCol='rgba(180,50,30,0.80)';
        else                          barCol='rgba(20,160,70,0.28)';
        ctx.fillStyle=barCol;
    ...
    const _hoverHits2=[];
    const mx2=_I?.mouseX||0, my2=_I?.mouseY||0;
...
export const RPANEL = { drawStartScreen, drawLogPanel, drawDcPanel, drawDamagePanel, drawCrewPanel, drawDamageScreen, drawPanel, drawEndScreen };
