(() => {
  ...
  // type: 'oneshot' — action fires once per keypress (consumed by I.justPressed)
  //       'held'    — action fires every frame the key is down (read via I.keys.has)
  //       'inline'  — handled directly in the keydown event handler (input.js)
  const KB = {
    // ── Simulation one-shots (consumed in sim.js) ──────────────────────────
    reload:         { key: 'r',      type: 'oneshot' },
    damageScreen:   { key: 'h',      type: 'oneshot' },
    damageScreenAlt:{ key: 'y',      type: 'oneshot' },  // alternate binding
