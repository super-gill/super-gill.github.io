<#
Comprehensive and bomb-proof module that automates the removal of users from SharePoint special, Teams groups
Converts user to shared
Removes users licenses [requires MGGraph :(]
Adds permissions to shared mailbox as required
Creates a comprehensive log of events for troubleshooting failures
Doesnt equires either local administrator or GA role 365 administrator (will require other admin roles though)
Offers all tasks as a selection to the script user
#>
...
# Add the ability to remove multiple users (see below for expanded list of related features)

# Expanded Features:
# Make the script able to remove Cove users without having to interact with Cove directly: 
# Add converting the user to shared
# Add removing all groups from the shared user (could this replace the teams removal?)
# Add a way to provide a list of users to be removed
# Add logging output file that includes each action, runas, hostname and errors

#Known issues:
# Fix: The script throws if the spo and teams modules are not installed, catching this and providing feedback would be better
# DONE: Fix: userConfirm does not recieve $adminURL and $adminUPN through the parameters on the function call
# DONE (?): Fix: the script always fails to connect to SPO first time and never asks for mauth login (might be the same for teams but is suppressed)

# Changes:
# Addded tolerance of name and version changes and some other details
# Re-named the script to CURT.ps1 (Cove User Removeal Tool)
...
# Removed HILARIOUS but not very professional messages to specific users
...
# Added a condition where the user selects not to change teams or SP
# Added multiple user functionality through a CSV that must be provided and logic to handle and convert it to an array
# Added the logging engine that is tolerant of missing paths or duplicate names (but havent added the log entries yet)
...
# If the user is not found its handled by catch as an error when its not really an error
# Adding removing all licenses from shared user requires MGGraph and either a GA or an admin with appreg roles
...
# My version of comment help, less good looking and more lines of code, the best kind of productivity.
if ($showHelp) {
...
function getOptions {
    ...
    if ($false -eq $userScope) {
        ...
        # Now $selectedUPNs contains the array of UPNs selected from the CSV
