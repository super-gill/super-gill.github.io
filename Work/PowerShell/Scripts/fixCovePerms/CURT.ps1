...
# FIXED: Fix: userConfirm does not recieve $adminURL and $adminUPN through the parameters on the function call
# Fix: the script always fails to connect to SPO first time and never asks for mauth login (might be the same for teams but is suppressed)
...
# If the user is not found its handled by catch as an error when its not really an error at all
# Add removing all licenses from shared user \\ this requires MGGraph
